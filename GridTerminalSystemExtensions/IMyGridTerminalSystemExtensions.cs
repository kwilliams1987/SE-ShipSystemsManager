...
namespace IngameScript
{
    static class IMyGridTerminalSystemExtensions
    {
        ...
        public static IEnumerable<T> GetZoneBlocks<T>(this IMyGridTerminalSystem grid, String zone, Boolean all = false)
            where T : class, IMyTerminalBlock => grid.GetBlocksOfType<T>(p => (p.IsWorking || all) && p.InZone(zone));
        ...
        public static IEnumerable<T> GetZoneBlocksByFunction<T>(this IMyGridTerminalSystem grid, String zone, String function, Boolean all = false)
            where T : class, IMyTerminalBlock => grid.GetZoneBlocks<T>(zone, all).Where(p => p.IsA(function));

        public static IEnumerable<String> GetZones(this IMyGridTerminalSystem grid) => grid.GetBlocksOfType<IMyTerminalBlock>().SelectMany(b => b.GetZones()).Distinct();

        public static Boolean AdjacentZonesTest<T>(this IMyGridTerminalSystem grid, Func<T, Boolean> test, params String[] zones)
            where T : class, IMyTerminalBlock => grid.GetBlocksOfType<T>(v => v.InAnyZone(zones)).All(test);
